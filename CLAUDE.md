...
The `planner`, `python-*`, `shell-*`, `skill-*`, and `data-*` agents are general tooling agents — they are not bound to the CRM domain specs and instead read the root `CLAUDE.md` plus, for shell, `docs/specs/SPECS-infrastructure.md`.
...
|------|-------|-------------|
| [`DOMAIN.md`](docs/specs/DOMAIN.md) | Business domain: entity meaning, relationships, delete behavior, sales pipeline, roles (no schema) | All 18 domain-bound agents (every agent except `planner` and the `python-*`, `shell-*`, `skill-*`, `data-*` tooling agents) |
