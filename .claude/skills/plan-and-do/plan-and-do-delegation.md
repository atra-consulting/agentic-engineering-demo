...
| New Spring Boot endpoint | `be-coder` | `sonnet` |
| Cross-cutting DB migration + schema redesign | `db-coder` | `opus` |
| Review a small Kotlin diff | `be-reviewer` | `haiku` |
...
The last four rows extend the package's own principles to dispatch points this project has that the package's source sibling did not — same rules, not new ones.
...
**A delegated fix is a slice like any other.** A test failure, a review finding, a failed browser scenario — each one becomes a slice, and each one runs this same loop. No shortcuts for fixes.
