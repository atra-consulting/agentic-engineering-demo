...
Every document opens with `## Summary`, containing `### Business Summary` first (2–4 sentences, business audience — no jargon, no file paths, no code), then `### Technical Summary` second (2–5 sentences, technical audience only). PRD — everything else stays business-readable, except Technical Summary and an optional trailing `## Technical Notes` section, both for technical readers only. Plan — everything else is technical-only; only the Business Summary needs to make sense to a business person.
...
- **Business** — `ba-writer`, `ba-reviewer`, `requirements-reviewer`
