...
Autonomous task queue. Tasks arrive from four external sources and move through a defined lifecycle: `OPEN → IN_PROGRESS → DONE | REJECTED`. Seeded idempotently on every startup via `INSERT OR IGNORE` with fixed ids 1–23 (`backend/src/seed/agentTaskSeed.ts`). Existing rows are otherwise left as-is on subsequent startups — except task #23's `title`, which a standing, unconditional `UPDATE` re-pins to its current canonical value on every `seedAgentTasks()` call.
