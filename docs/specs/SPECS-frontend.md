...
A short static intro (title + one-line subtitle) sits above a persistent disclosure note, which sits above a single tab strip (NgbNav) with four tabs, one per process, in that order. The disclosure note states two things in German: the shown times are example values, and every value — plus the step count itself — can be changed. It cannot be dismissed, does not auto-hide, and stays the first content block on a narrow viewport, above both the tabs and the Prozessvergleich card. Each tab shows: its Arbeit/Warten bar (default view) with a Balken ↔ Flussdiagramm toggle button, an Arbeit-vs-Warten pie (all four tabs), a BA/Dev/Tester role-split pie (Agile mit Menschen and Agile mit KI tabs only), and that process's own Schritt-Zeiten step-time form. There is no second, competing tab strip — the outer tab is the only process picker.
...
Each process's step count is live, not fixed. A "Schritt hinzufügen" button below the last step row appends a step (floor 1, cap 50 per process); every step row carries its own "Entfernen" button. 19/19/11/2 are now only the *default* step counts a fresh page load shows — `stepCount` in `prozess-defaults.ts` seeds the initial form, it no longer caps it. A blocked add/remove control (at the floor or the cap) stays focusable and carries `aria-disabled="true"` with a persistently visible reason next to it, instead of the native `disabled` attribute (visual pattern: `docs/specs/SPECS-ui.md`). Add/remove also announces the new step count through a polite, per-process `aria-live` region — the first purpose-built `aria-live` region for announcing a user action's result in this codebase (pattern: `docs/specs/SPECS-ui.md`). Focus never gets lost: after add it moves to the new step's name field; after remove, focus goes to the add button if that removal hit the floor; otherwise to the new last step if the removed step was last; otherwise to the step that shifted into the removed step's position.

Each step also carries its own editable name, seeded from the existing German label (a newly added step defaults to "Neuer Schritt"). An empty name displays as "Schritt N" using the step's current position, everywhere the name is shown — the printed row, chart tooltips, the flow diagram, and screen-reader text. On the two agile tabs, every step row additionally carries a role picker (BA / Dev / Tester / "— Keine —"); a newly added step starts with no role. The role-split pie's note always states that only steps with a role count, and names the excluded work minutes when a role-less step carries time.
...
Below the four tabs, in its own section: the Prozessvergleich card — a shared-scale comparison bar chart across all four processes. Each row also shows a caption (the process's roles) and two Annahmen bullets, both sourced from `prozess-defaults.ts` (`PROZESS_CAPTION`, `PROZESS_ANNAHMEN`). Clicking a row (or pressing Enter/Space) switches to that process's tab. A filter button cycles how many bars show: alle, 1, 1–2, 1–3. The chosen limit persists across navigation (`sessionStorage`, key `rechner.barLimit`) — but only on an explicit click of the filter button. Tab switches and picking a process from the chart can widen the visible bars without persisting that change. Below the Prozessvergleich card, in its own card (collapsed by default, click to expand): the Szenarien save/load/delete flow. Saving sends each process's current step count, works, waits and names; loading rebuilds the form to the stored length (falling back to that process's defaults when the stored shape is structurally invalid) and re-derives the agile role state to match. Bars, pies, and the flowchart are all generated on the fly via pure functions in `svg-util.ts` — no charting library. Named scenarios persisted via `SzenarioService` → `/api/szenarien` (create/load/update/delete, `ConfirmDialogComponent` for delete). Duration formatting via the `dauer` pipe (8h workday). Sidebar: "Produktivität" section, item "Rechner" (`faCalculator`), no role required.
