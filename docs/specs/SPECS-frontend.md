...
Each process's step count is live, not fixed. A "Schritt hinzufügen" button below the last step row appends a step (floor 1, cap 50 per process); every step row carries its own "Entfernen" button. 19/19/11/2 are now only the *default* step counts a fresh page load shows — `stepCount` in `prozess-defaults.ts` seeds the initial form, it no longer caps it. A blocked add/remove control (at the floor or the cap) stays focusable and carries `aria-disabled="true"` with a persistently visible reason next to it, instead of the native `disabled` attribute (visual pattern: `docs/specs/SPECS-ui.md`). Add/remove also announces the new step count through a polite, per-process `aria-live` region — the only use of `aria-live` in this codebase (pattern: `docs/specs/SPECS-ui.md`). Focus never gets lost: after add it moves to the new step's name field; after remove it moves to the step now in that position, the new last step if the removed step was last, or the add button if removal hit the floor.
