...
- [ ] In `.claude/agents/planner.md`, line 16 (currently: "Every document opens with a `## Summary`. PRD — 2–4 sentences, business audience. Plan — 2–5 sentences, technical audience, followed by a `## Business Summary` section — 2–4 sentences, business audience."): replace the whole line with this exact text:
  ...
  ```
- [ ] In the same file, `### 9c. Complete copy-paste agent file` section (an embedded copy of a planner.md agent definition inside a fenced code block, around line 208) and `### 9d. What the prompt must cover` (a checklist bullet, around line 300): both still describe the OLD flat summary rule. Update both to match the corrected rule above — the `### 9c` line becomes the same exact text as the `planner.md` replacement above; the `### 9d` bullet becomes: "**Every document opens with `## Summary`: `### Business Summary` first, `### Technical Summary` second.** PRD stays business-readable except Technical Summary and an optional Technical Notes section. Plan is technical-only except its Business Summary."
- [ ] Do not touch `.claude/skills/review/SKILL.md` — reviews get only the writing-style scope note (already done), not the business/technical summary split. The user's request scoped the summary split to PRDs and plans only (see Business Summary above); reviews were never in scope for that part.
