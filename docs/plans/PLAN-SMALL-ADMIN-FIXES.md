...
## Summary
### Business Summary
Three small fixes to the admin tools. The Kanban ticket board will show each ticket's number on its card, not just inside the ticket. The `/project:write-ticket` automation already leaves a text mention of the ticket number on the original feedback item; it will now upgrade that to a clickable link back to the ticket, matching the link the ticket already carries back to the feedback. Agent-task #23 will show its German title, "Chancen verbessern," instead of an English placeholder — and this fix will correct any environment, including production, on its next restart.
