...
Diese App hat drei Tools für die Demo. Alle laufen lokal. Starte die App mit `./start.sh`. Dann öffne die URL im Browser.
...
**Wozu?** Das Tool zeigt den Produktivitätsgewinn durch KI. Gut für Vorträge und Demos.
...
**Hinweis für Demos.** Filter nach einer Quelle, z. B. <http://localhost:7200/admin/agent-tasks?source=EMAIL>. Meist sind nur die **obersten zwei, drei** Einträge wirklich noch offen. Die älteren stehen zwar auch auf „Open", doch frühere automatische GitHub-Actions-Läufe haben sie schon abgearbeitet. Wie passt das zusammen? `POST /reset` setzt den Status aller Tasks zurück auf `OPEN` — die Ergebnisse (Tickets, PRs) aus diesen Läufen bleiben aber. Für eine Demo nimmst du einen der obersten. (Frisch lokal gestartet stehen dagegen alle 23 echt auf `OPEN` — dort ist noch nichts gelaufen.)

Den Skill, der eine Task zu einem Kanban-Ticket triagiert, findest du unter [`/write-ticket`](../README.MD#write-ticket--feedback-in-ein-neues-ticket-triagieren).

**Wozu?** Das Tool zeigt einen autonomen Agenten im „Software-Factory"-Betrieb.
...
Ein Kanban-Board. Ein einfaches Ticketsystem für die Software-Factory-Demo.
...
- **Seed:** 12 Demo-Tickets. Läuft bei jedem Start (`INSERT OR IGNORE`, ids 1–12). `POST /reset` löscht alles und baut die 12 neu auf.
