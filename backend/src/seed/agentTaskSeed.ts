...
export async function seedAgentTasks(): Promise<void> {
  ...
  // REQ-302: INSERT OR IGNORE above never touches a row that already exists,
  // so an already-seeded database (any developer's existing local SQLite
  // file, or a deployed Turso database) never picks up the corrected id-23
  // title just because AGENT_TASK_SEED changed. Run a one-time corrective
  // UPDATE on every startup, guarded so it fires only while id 23 still
  // carries the old English literal. Sourcing the replacement value from
  ...
  // to some other value is left untouched for the same reason. Only the
  // title column is written — status/comment/pickedUpAt/resolvedAt on a row
  // that has since moved to DONE through real usage stay exactly as they
  // are.
